...
ext = {'*.jpg', '*.png', '*.bmp'};
scale = 2;
record_results_txt = ['D:\matlib\PSNR_SSIM_Results_', degradation,'_model.txt'];
results = fopen(fullfile(record_results_txt), 'wt');  

fprintf(results, '**********************\n');
filepaths = [];
for idx_ext = 1:length(ext)
    filepaths = cat(1, filepaths, dir(fullfile('E:\SR data\HSC-SDSS\HSC-38\n=2000\normalize\HSC_test_HR',ext{idx_ext})));
end
...
for idx_im = 1:length(filepaths)
    ...
    im_HR = imread(fullfile('E:\SR data\HSC-SDSS\HSC-38\n=2000\normalize\HSC_test_HR',name_HR));
    im_SR = imread(fullfile('E:\SR data\HSC-SDSS\HSC-38\test-50\esrgan_test_output',name_HR));
    ...
    fprintf(results, 'x%d %d: PSNR= %f SSIM= %f\n', scale, idx_im,  PSNR_all(idx_im), SSIM_all(idx_im));
